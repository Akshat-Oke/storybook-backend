...
export const ownPosts = async (req, res, next) => {
  ...
  try {
    console.log("req.userId", req.userId, " and req.user._id", req.user._id);
    const userPosts = await postMessage
      .find({
...
export const updatePost = async (req, res, next) => {
  const { id: _id } = req.params; //rename id as _id
  const post = req.body;
...
export const findPost = async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(_id)) {
    return res.json({
      ...
      message: "Check your link again",
      createdAt: new Date().toISOString(),
      selectedFile: { base64: "data:image/png;base64," + img_64 },
      creator: { name: "StoryBook server" },
      _id: _id,
    });
  }
  try {
    ...
    if (post) res.json(post);
      ...
      res.json({
        ...
        createdAt: new Date().toISOString(),
        selectedFile: { base64: "data:image/png;base64," + img_64 },
        creator: { name: "the StoryBook server" },
        _id: _id,
      });
  } catch (e) {
    res.json({
      ...
      message: "Check your link again",
      createdAt: new Date().toISOString(),
      selectedFile: "data:image/png;base64," + img_64,
      creator: { name: "the StoryBook server" },
      _id: _id,
    });
  }
...
export const likePost = async (req, res) => {
  const { id } = req.params;
  if (!req.userId) return res.status(404).json({ message: "Not authorized" });
